...
/*
 * TODO: Notes taken during call with Ye. To be deleted ...
 *
 * I need to probably do e2e implementation like this for CCA:
 *
 * Option 1: (John prefers this option)
 *
 *  - Figure out how to write this fn. How to verify evidence provided by CCA-shim?
 *  - CCA-shim provides an attest function which generates an evidence report
 *  - Need to write something to generate an evidence package on top of that evidence.
 *  - Write a fn that calls CCA-Attest(), packages it together before sending it to
 *    the Cert service.
 *
 *  - certify_me() will serialize evidence package and send it to Cert Service
 *  - In this fn, VerifyKeystoneAttestation() seems to take serialized byte stream
 *    and a key. The former seems to be serialiazed evidence.
 *
 ...
 *
 *    We can use SEV-implementation as a reference here
 *
 ...
 *  - Dev VerifyCCAAttestation () which will call islet_verify()
 *
 *    (Approach Done by OE and Gramine. both use Intel DCAP. therefore, we don't
 *      need the Cert as DCAP will grab the cert itself.)
 *      See: VerifyGramineAttestation() -> gramineverify.GramineVerify(),
 *           which only takes serialized evidence and not a key.
 *
 * Choice Depends on if Samsung has support for option-1 / 2.
 *
 * Meanwhile, I can work on 'client-side'; i.e.
 * Finish packaging the evidence into a package on client-side. certify_me()
 *
 * - Get stuff working for certify_me() ... to implement all this, cold_init(), ...
